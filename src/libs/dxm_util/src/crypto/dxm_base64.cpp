...
void
dxm_base64_encode_init (struct dxm_base64_state *state)
{
	state->eof = 0;
	state->bytes = 0;
	state->carry = 0;
...
void
dxm_base64_encode_update (struct dxm_base64_state *state, const char *const src, dxm_uint32 srclen, char *const out, dxm_uint32 *const outlen)
{
...
void
dxm_base64_encode_final (struct dxm_base64_state *state, char *const out, dxm_uint32 *const outlen)
{
	char *o = out;
...
void
dxm_base64_decode_init (struct dxm_base64_state *state)
{
	state->eof = 0;
	state->bytes = 0;
	state->carry = 0;
...
int
dxm_base64_decode_update (struct dxm_base64_state *state, const char *const src, dxm_uint32 srclen, char *const out, dxm_uint32 *const outlen)
{
	int ret = 0;
	const char *c = src;
	char *o = out;
...
void
dxm_base64_encode (const char *const src, dxm_uint32 srclen, char *const out, dxm_uint32 *const outlen)
{
	dxm_uint32 s;
	dxm_uint32 t;
	struct dxm_base64_state state;

	/* Init the stream reader: */
	dxm_base64_encode_init(&state);

	/* Feed the whole string to the stream reader: */
	dxm_base64_encode_update(&state, src, srclen, out, &s);

	/* Finalize the stream by writing trailer if any: */
	dxm_base64_encode_final(&state, out + s, &t);
...
int
dxm_base64_decode (const char *const src, dxm_uint32 srclen, char *const out, dxm_uint32 *outlen)
{
	struct dxm_base64_state state;

	dxm_base64_decode_init(&state);
	return dxm_base64_decode_update(&state, src, srclen, out, outlen);
}
